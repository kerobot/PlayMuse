...
**最終更新**: 2026-01-27 17:00:00

**追加要件（プレイリスト関連タスク前）**:
- WAV / AAC ファイル対応
- プレイリストドラッグ＆ドロップ対応
- プレイリスト表示拡張（アルバムアート、詳細メタデータ表示）
- 曲削除ボタン追加
...
- ✅ **ループ再生機能を実装する。`IPlaylistService` に `IsLoopEnabled` プロパティを追加し、有効時はプレイリスト末尾でNextを実行すると先頭に戻り、先頭でPreviousを実行すると末尾に移動するよう変更する。**
...
- ✅ **プレイリスト各曲の右端に曲削除ボタンを追加する。ボタン押下時に `IPlaylistService.Remove()` を呼び出し、再生中の曲が削除された場合は再生を停止して次の曲へ移動するロジック（または停止状態を維持するか明確化）を実装する。**
...
- ✅ **プレイリストのローカル保存/読込機能を実装する。`IPlaylistService` に `SavePlaylist(string filePath)` / `LoadPlaylist(string filePath)` メソッドを追加し、トラックファイルの”絶対パス”リストをJSON形式（例: `.plm` 拡張子）で保存/読込できるようにする。**
- ✅ **プレイリスト保存/読込のUIを追加する：「プレイリストを保存」「プレイリストを開く」ボタンを「開く」ボタンの右側にアイコンボタンとして配置し、`SaveFileDialog` / `OpenFileDialog` でファイル選択を行う。フィルタは `*.plm` とする。**
- ✅ **アプリ終了時に開いていたプレイリストファイルを記憶し、次回起動時に自動読込する。`ISettingsService` に `LastPlaylistFilePath` プロパティを追加し、終了時に保存、起動時に存在確認後に読込する。読込時にプレイリストや音楽ファイルが存在しない場合は警告表示してスキップする。**
...
- ✅ **動的な環境変化への耐性を強化する：現在の実装を確認したうえで、プレイリスト追加後にファイルが削除/移動された場合や、再生中に出力デバイスが切断された場合を検知し、未処理例外ではなく回復可能なエラー状態として扱う。ファイル削除や移動はスキップして再生を継続、デバイス切断は他のアクティブなデバイスに切り替えて再生を継続する**
